package az.ingress.mscard.common;
...
public class LoggingAspect {
    ...
    @Before("execution(* az.ingress.mscard.*.*.*.*(..))")
    ...
    @AfterReturning(pointcut = "execution(* az.ingress.mscard.*.*.*.*.*(..))", returning = "result")
    ...
    @AfterThrowing(pointcut = "execution(* az.ingress.mscard.*.*.*.*(..))", throwing = "exception")
